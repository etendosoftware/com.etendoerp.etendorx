...
public class SelectorHandlerUtil {
    ...
    // An HQL identifier and a dotted HQL path, checked before either is concatenated into a query.
    private static final String HQL_ALIAS_PATTERN = "[A-Za-z_][A-Za-z0-9_]*";
    private static final String HQL_PATH_PATTERN = HQL_ALIAS_PATTERN + "(\\." + HQL_ALIAS_PATTERN + ")+";
    ...
    private static String resolveRecordIdExpression(Selector selectorDefined) {
        ...
        if (valueField != null && StringUtils.isNotBlank(valueField.getClauseLeftPart())) {
            ...
            return clauseLeftPart.matches(HQL_PATH_PATTERN) ? clauseLeftPart : null;
        }
